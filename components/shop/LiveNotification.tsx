...
// Donnees de demonstration en fallback si la DB est vide
const demoNotifications: Notification[] = [
  { productName: 'Ecouteurs Bluetooth Pro', customerName: 'Kevin', quarter: 'Cadjehoun' },
  { productName: 'Montre Connectee Sport', customerName: 'Amina', quarter: 'Akpakpa' },
  { productName: 'Sac a Main Elegant', customerName: 'Fatou', quarter: 'Fidjrosse' },
  { productName: 'Baskets Running', customerName: 'Ibrahim', quarter: 'Haie Vive' },
  { productName: 'Parfum Oriental', customerName: 'Marie', quarter: 'Godomey' },
]
...
export default function LiveNotification() {
  ...
  // Fetch les notifications depuis la DB au montage
  useEffect(() => {
    async function fetchNotifications() {
      try {
        ...
        if (res.ok) {
          ...
          // Utiliser les notifications DB si disponibles, sinon garder le fallback demo
          if (data.length > 0) {
            setNotifications(data)
          }
        }
      } catch {
        // En cas d'erreur reseau, on garde les notifications demo
      }
    }
  ...
  return (
    <div className="fixed bottom-20 md:bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-80 z-30 animate-slide-up">
