...
export default function ProductReviews({ productId }: { productId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!isValidBeninPhone(formData.phone)) return
        ...
        <form onSubmit={handleSubmit} className="bg-warm-50 rounded-xl p-4 mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              className="px-3 py-2 text-sm rounded-lg border border-warm-200 focus:outline-none focus:ring-2 focus:ring-primary-500"
              required
            />
            <input
              type="tel"
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-warm-600">Note :</span>
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((s) => (
                ...
                >
                  <svg
                    className={`w-6 h-6 transition-colors ${s <= formData.rating ? 'text-accent-500' : 'text-warm-200'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
